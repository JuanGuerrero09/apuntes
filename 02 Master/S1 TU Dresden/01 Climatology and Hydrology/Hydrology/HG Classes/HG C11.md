...
- **What is a "design flood" and what is its engineering purpose?**
    
    - A design flood is a hypothetical peak discharge or hydrograph (e.g., $HQ_{100}$) determined by statistical analysis. It is used as a safety benchmark for designing hydraulic structures like dams, dikes, and spillways.
        
- **Name meteorological and catastrophic causes of a flood.**
    
    - **Meteorological:** Extreme rainfall, rapid snowmelt, storm surges, or ice jams. **Catastrophic:** Dam or levee failures, and tsunamis caused by earthquakes.
        
- **Differentiate between the three main types of droughts.**
    
    - **Meteorological:** Deficiency in precipitation. **Agricultural:** Deficiency in soil moisture affecting crops. **Hydrological:** Deficiency in streamflow, reservoir levels, and groundwater tables.
        
- **What are the primary triggers for a drought event?**
    
    - Prolonged precipitation deficits often coupled with high temperatures and high solar radiation, which increase evapotranspiration and deplete soil and water storage.
        
- **Define floods and droughts in hydrological terms.**
    
    - A **flood** is a high-flow event where discharge rises significantly above the mean for a **short duration**. A **drought** is a low-flow event where discharge falls below a critical threshold for an **extended period**.
        
- **Which flood parameters are used as design criteria for protection structures?**
    
    - Peak water level (for dike height), peak discharge (for canal capacity), flood duration (for safety), and flood volume (for retention basin size).
        
- **What are the three main assumptions of the Flow Duration Curve?**
    
    - 1. The data record is continuous and representative. 2. Observed fluctuations are random with a stable mean (stationarity). 3. Variations are driven solely by climate, not land-use change.
            
- **How is a design flood ($HQ$) estimated for an ungauged cross-section?**
    
    - By using the **Reference $HQ$-Method**, where data is transferred from a "donor" catchment with similar physical characteristics (soil, land use, morphology).
        
- **Name the criteria used to quantify the severity of a drought.**
    
    - Critical low flow ($Q_{crit}$), duration of the deficit, deficit volume (total water missing), and drought intensity (severity divided by duration).
