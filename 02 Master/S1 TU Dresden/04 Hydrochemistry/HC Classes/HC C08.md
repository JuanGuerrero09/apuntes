...
![[Pasted image 20260120163912.png]]
...
Set of equations for bases: 


- **Law of Mass Action for Base Dissociation ($\text{K}_\text{a}$):**
$$ \text{K}_\text{b} = \frac{c(\text{BH}^+)\, c(\text{OH}^-)}{c(\text{B})}$$
- **Law of Mass Action for Water Dissociation ($\text{K}_\text{w}$):**$$ \text{K}_\text{w} = c(\text{H}^+)\, c(\text{OH}^-)$$
- **Material Balance Equation:** States that the total initial concentration of the base ($\text{C}_0(\text{Base})$) must equal the sum of the concentrations of all species derived from that base in the solution (dissociated and undissociated form):$$ \text{C}_0(\text{Base}) = c(\text{B}) + c(\text{BH}^+)$$
- **Charge Balance Equation:** States that the sum of the concentrations of all positive charges (cations) in the solution must equal the sum of the concentrations of all negative charges (anions). For a simple acid ($\text{HA}$) solution:$$ c(\text{OH}^-) = c(\text{H}^+) + c(\text{BH}^+)$$
...
$$
c(\text{OH}^-) = \frac{-\text{K}_\text{b}}{2} + \sqrt{\frac{\text{K}_\text{b}^2}{4} + \text{K}_\text{b}\text{C}_0}
$$
...
**Strong** (only see the reaction, don't interact with water)

If you see these ions in a salt, cross them out immediately; they do not change the pH.

- **From Strong Acids (Anions):** $\text{Cl}^-$ (Chloride), $\text{Br}^-$ (Bromide), $\text{I}^-$ (Iodide), $\text{NO}_3^-$ (Nitrate), $\text{SO}_4^{2-}$ (Sulfate), $\text{ClO}_4^-$ (Perchlorate).
- **From Strong Bases (Cations):** Group 1 and 2 metals.2 $\text{Li}^+$, $\text{Na}^+$ (Sodium), $\text{K}^+$ (Potassium), $\text{Mg}^{2+}$, $\text{Ca}^{2+}$.

**Weak** (Actively interact with water)

If you see these, they are the ones that react with water to change the pH.

- **From Weak Acids (These act as Bases):** $\text{Ac}^-$ (Acetate), $\text{F}^-$ (Fluoride), $\text{NO}_2^-$ (Nitrite), $\text{CN}^-$ (Cyanide), $\text{CO}_3^{2-}$ (Carbonate), $\text{PO}_4^{3-}$ (Phosphate).
- **From Weak Bases (These act as Acids):** $\text{NH}_4^+$ (Ammonium), or any metal that is **not** in Group 1 or 2 (like $\text{Al}^{3+}$ or $\text{Fe}^{3+}$).
...
- The ranges for getting pH are:
	- *Very strong acids* ($\text{pK}_\text{a} < -1.74$)
	- *Strong acids* ($\text{pK}_\text{a} = -1.74 \text{ to } 4.5$)
	- *Medium and weak acids* ($\text{pK}_\text{a} > 4.5$)
	- Very strong bases
	- Strong bases
	- Weak and medium strong bases
...
- Under what specific conditions is the simple weak acid formula ($\text{pH} = 0.5 (\text{pK}_\text{a} - \lg \text{C}_0)$) valid, and what chemical assumption does it rely upon?
	- It relies on the assumption that the acid dissociates so little that $[HA]_{equilibrium} \approx C_0$. Mathematically, this is valid if the degree of dissociation $\alpha < 0.05$ (the **5% rule**). If $\alpha$ is higher, you must use the quadratic formula.
- Explain the difference in chemical mechanism between the $\text{pH}$ change caused by dissolving $\text{HCl}$ (a strong acid) versus $\text{NH}_4\text{Cl}$ (an acidic salt).
	- 1$HCl$ is a **protolysis** involving a strong acid that dissociates 2$100\%$.3 $NH_4Cl$ causes a pH change via **hydrolysis**. The 4$NH_4^+$ ion (conjugate acid of a weak base) reacts with water to release 5$H^+$, but only to a small extent governed by its 6$K_a$.7
- Why does the quadratic formula for strong acids simplify the charge balance by ignoring the concentration of $\text{OH}^-$?
	- In acidic solutions (where $pH < 6$), the concentration of $OH^-$ is negligible (less than $10^{-8}\text{ M}$). Because $[H^+]$ will be significantly larger than $[OH^-]$, the term $[OH^-]$ in the charge balance $[H^+] = [A^-] + [OH^-]$ can be ignored without affecting the final result.
